...
***************************************************************************************************/
CREATE PROC [dbo].[RPT_AXP_AA] @BRANCH_DATE DATE
AS
    ...
    A.DATE_KEY
		...
		,A.CVG_MPREM_AMT
		,A.CVG_SUM_INS_AMT
		,AFYP = CASE WHEN A.PLAN_ID = 'CBS001' THEN A.AFYP * 0.1 ELSE 
        CASE WHEN A.PLAN_ID = 'BES001' THEN CVG.CVG_UNDSCNT_PREM * (CASE WHEN CAST(A.POL_BILL_MODE_CD AS INT)>0 THEN 12/CAST(A.POL_BILL_MODE_CD AS INT) ELSE 1 END) ELSE A.AFYP END END
		--,A.AFYP 
		
        
        ,WFYP = CASE WHEN P.PLAN_ID = 'KBE003' THEN 
        CASE WHEN (P.POL_BILL_TYP_CD = 'P' AND A.POL_PREM_PAY_DUR in (15,12,9,6)) or A.POL_PREM_PAY_DUR = 6
         THEN DLKHSTAGE.dbo.FN_GET_WFYP (A.POL_ID, A.POL_PREM_PAY_DUR, A.CVG_MPREM_AMT )/2
         ELSE DLKHSTAGE.dbo.FN_GET_WFYP (A.POL_ID, A.POL_PREM_PAY_DUR,  A.CVG_MPREM_AMT )
        END
        ELSE DLKHSTAGE.dbo.FN_GET_WFYP (A.POL_ID, A.POL_PREM_PAY_DUR, CASE WHEN A.PLAN_ID = 'BES001' THEN CVG.CVG_UNDSCNT_PREM ELSE A.CVG_MPREM_AMT END ) END
	...
	LEFT JOIN DLKHSTAGE..SGTB_POLICY_CLIENT PC ON PC.POL_ID = FA.POL_ID 
